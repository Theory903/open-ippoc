...
class GraphManager:
    ...
    async def _find_paths_cte(
        ...
        # Recursive CTE query
        # Optimization: We use simple string concatenation for path tracking which is portable between Postgres and SQLite
        # and execute path finding as a native CTE to avoid N+1 application-level queries
        cte_query = text("""
        ...
        # Optimization: Collect all unique node IDs to fetch names in bulk instead of individual lookups
        all_node_ids = set()
        parsed_rows = []

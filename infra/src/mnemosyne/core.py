    ...
    async def forget(self, criteria: Dict[str, Any]) -> int:
        ...
        Args:
            criteria: Deletion criteria (implementation-dependent)
                Expected format:
                {
                    ...
                    "semantic": {"ids": [...]},
                    "procedural": {"skills": ["skill_name", ...], "skill_name": "skill_name"},
                    "graph": {"entities": ["entity_name", ...], "entity_name": "entity_name"}
                }
            ...
            try:
                ...
                if "ids" in semantic_criteria:
                    ...
                    if semantic_ids:
                        ...
                        count = res if isinstance(res, int) else (len(semantic_ids) if res else 0)
                        total_deleted += count
            except Exception as e:
        ...
        if "procedural" in criteria and self.procedural:
            try:
                ...
                skills_val = proc_criteria.get("skills", [])
                skills = list(skills_val) if isinstance(skills_val, list) else [skills_val] if isinstance(skills_val, str) else []
                ...
                for skill_name in skills:
                    ...
                    count = res if isinstance(res, int) else (1 if res else 0)
                    total_deleted += count
            except Exception as e:
        ...
        if "graph" in criteria:
            try:
                ...
                entities_val = graph_criteria.get("entities", [])
                entities = list(entities_val) if isinstance(entities_val, list) else [entities_val] if isinstance(entities_val, str) else []

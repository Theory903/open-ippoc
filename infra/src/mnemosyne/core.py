    ...
    async def forget(self, criteria: Dict[str, Any]) -> int:
        ...
        if "procedural" in criteria and self.procedural:
            try:
                ...
                skills = proc_criteria.get("skills") or proc_criteria.get("skill_name")
                if skills:
        ...
        if "graph" in criteria:
            try:
                ...
                entities = graph_criteria.get("entities") or graph_criteria.get("entity_name")
                if entities:
                    if isinstance(entities, str):
                        entities = [entities]
                    for entity in entities:
                        count = await self.graph.delete_entity(entity)
                        total_deleted += count if type(count) is int and not isinstance(count, bool) else (1 if count else 0)
            except Exception as e:

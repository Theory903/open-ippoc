...
from cortex.core.queue import get_planner_queue
...
class Planner:
    ...
    def plan(self, observation: Dict[str, Any], intents: IntentStack) -> Optional[Intent]:
        ...
        # External requests are processed before the cycle in AutonomyController.run_cycle()
        # and injected into the intent stack.
            ...
            for msg_id, payload in items:
                try:
                    ...
                    if envelope_json:
                        ...
                        # Ensure external requests are classified as Duty unless specified otherwise
                        if "intent_type" not in data:
                            data["intent_type"] = IntentType.SERVE
                        else:
                            try:
                                data["intent_type"] = IntentType(data["intent_type"])
                            except ValueError:
                                print(f"[Autonomy] Invalid intent_type: {data['intent_type']}, defaulting to SERVE")
                                data["intent_type"] = IntentType.SERVE

        ...
        action: (signal) => {
          ...
          try {
             ...
             import("child_process").then(cp => {
                 // Security Fix: Replaced vulnerable cp.exec with cp.execFile to prevent command injection
                 cp.execFile('renice', ['+10', '-p', String(signal.payload.pid)], (error) => { if (error) console.error('renice error:', error); });
             });

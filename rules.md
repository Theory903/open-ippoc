...
2.5 Security Surface Rule (NEW - Refinement 1)

Core security invariants (identity, admission, replay, trust) MUST NOT be disabled via feature flags in production builds. Feature gating is acceptable only for non-critical protocol extensions and must be OFF by default.
...
6.1 Dead-Code Elimination Rule (NEW - Refinement 2)

Dead-code elimination MUST NOT be mixed with behavioral changes in the same PR. cleanup PRs must be atomic to keep evolutionary diffs auditable and prevent "smuggling" of logic changes.

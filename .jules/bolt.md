...
**Learning:** Calculating graph similarity metrics (like Jaccard) using N+1 full table scans in Python loops causes massive latency degradation as graph size increases.
**Action:** Use intersection-first optimization within SQL via Common Table Expressions (CTEs) to filter candidates by shared relations and compute similarity directly in the database.

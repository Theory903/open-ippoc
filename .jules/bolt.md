...
## 2024-03-24 - [Optimize intent classification generator loops]
**Learning:** Found an inline generator `any()` string match that runs much slower than explicitly unrolled for loops or tuple loops in RAG routers. Substring matching is ~3x faster when extracted to static class variables and explicit loops.
**Action:** Replace `any(word in query for word in [...])` generator expressions with explicit loops over statically defined tuple constants for string matching.

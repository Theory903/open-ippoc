...
## 2025-02-12 - Hoisting array initialization and unrolling Python generator expressions
**Learning:** Generator expressions with `any(...)` or `sum(...)` over dynamically allocated lists inside hot path routing methods cause substantial overhead due to repeated object creation and Python loop evaluation overhead.
**Action:** When optimizing repeated substring/keyword checking loops, declare lists as static class-level tuples (hoisting invariant operations) and replace generator expressions with explicit, unrolled `for` loops with early return/break to avoid implicit function call overhead and early termination.

...
## 2024-05-24 - N+1 Queries in Jaccard Similarity Search
**Learning:** Fetching relationships for all entities in a loop to compute Jaccard similarity in Python causes a massive O(N) query bottleneck, especially as the knowledge graph grows.
**Action:** Push complex intersection math and relationship candidate filtering directly into the database using SQL CTEs to collapse N+1 queries into a single database trip.

...
## 2024-05-24 - Optimize RAG Router string matching overhead
**Learning:** Using generator expressions with inline lists for string matching (`any(word in query for word in ['a', 'b'])`) creates measurable overhead. Moving to class-level tuple constants and unrolled for-loops provides a ~68% performance gain on RAG routing hotpaths, while set intersection breaks exact substring functionality.
**Action:** Default to unrolled loops with tuple constants for repeated substring matching on hotpaths instead of relying on `any()` generator comprehensions.

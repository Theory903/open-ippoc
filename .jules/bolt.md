...
## 2025-05-18 - Avoid N+1 Queries in Graph BFS Path Finding
**Learning:** Performing recursive path-finding with standard Python `deque` BFS loop and single `SELECT` database queries per depth causes severe N+1 database round trips.
**Action:** Always delegate deep tree/graph traversals directly to the database using `WITH RECURSIVE` CTEs when supported by SQL engine.

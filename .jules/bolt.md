...
## 2026-02-28 - Optimize Intent Classification
**Learning:** Generator expressions with substring matches (e.g., `any(word in query for word in [...])`) create significant setup/teardown overhead when evaluated on hot paths like router intent classification.
**Action:** Replace generators with explicit unrolled `for` loops over static class-level tuples for static keyword checks to prevent repeated list allocation and avoid function call overhead.

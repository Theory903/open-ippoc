...
## 2024-05-24 - String Matching and Regex Compilation Overheads
**Learning:** Re-evaluating inline generators, lists, and uncompiled regular expressions in heavily trafficked paths (like RAG extraction and Router evaluation) causes unnecessary GC overhead and execution latency. For example, recreating lists and using `any()` with strings inside loops takes ~0.33s per 10k operations, while hoisted tuples and `.endswith(tuple)` reduces this to ~0.17s.
**Action:** Always hoist invariant lists and sets as class-level tuple constants. Pass tuples directly to `str.endswith()` instead of using generator comprehensions for string suffix matching. Compile regex patterns (`re.compile`) globally if they are repeatedly evaluated.

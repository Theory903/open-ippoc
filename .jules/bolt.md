...
## 2026-06-21 - N+1 Query in Graph BFS Path Finding
**Learning:** Python-level BFS to traverse relational knowledge graphs results in an N+1 query problem, fetching relations at each depth and dramatically impacting performance as the branching factor increases.
**Action:** Always replace Python-level depth-first or breadth-first iterative graph queries with native database capabilities like Recursive CTEs (Common Table Expressions) and bulk-fetch patterns to delegate the traversal work to the database engine.

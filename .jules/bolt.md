...
## 2024-05-24 - [Optimize String Prefix/Suffix Matching]
**Learning:** Checking string extensions using `any(string.endswith(ext) for ext in set_of_extensions)` is inefficient because it iterates with a generator and repeatedly calls `endswith()`. It's a known performance anti-pattern.
**Action:** Use `string.endswith(tuple_of_extensions)` to pass the multiple options directly to the optimized C-level implementation of the string method, avoiding set creation and generator overhead.

...
## 2024-05-24 - dataclasses.asdict overhead in high-frequency paths
**Learning:** `dataclasses.asdict` performs deep copies by default, creating significant overhead (~3.2ms vs ~0.14ms per 10000 calls) when tracking state updates in hot paths like the `EconomyManager`.
**Action:** Manually construct dictionaries with shallow `.copy()` for nested structures instead of using `dataclasses.asdict` for performance-critical serialization.

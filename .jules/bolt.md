...
## 2024-04-11 - Optimize AdaptiveRAGRouter Intent Classification
**Learning:** Using explicit `for` loops over tuple constants instead of generator expressions with `any()` or `sum()` significantly improves execution speed for routing analysis in the `AdaptiveRAGRouter`. Unrolling the operations avoids generator/closure overhead and allows for early exits while maintaining exact substring match behavior.
**Action:** When implementing classification or routing logic that checks strings against many keywords, use static tuple constants and explicit `for` loops with early exits rather than generator comprehensions.

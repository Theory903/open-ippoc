...
## 2024-05-23 - [AdaptiveRAGRouter String Matching]
**Learning:** In AdaptiveRAGRouter, replacing generator expressions \`any(word in query for word in words)\` with explicit unrolled \`for\` loops and hoisting invariant \`query.lower()\` operations significantly reduces overhead.
**Action:** Use static class-level tuples and explicit unrolled loops for repetitive string matching instead of dynamic list creation and generator expressions.

...
## 2025-02-14 - Optimize Graph Path Finding N+1 Query
**Learning:** Iterative BFS in application code for graph pathfinding leads to severe N+1 query bottlenecks as each outgoing node expansion requires a separate database call.
**Action:** Always prefer SQL Recursive CTEs (WITH RECURSIVE) for bounded-depth graph pathfinding to allow the database to optimize and execute the entire traversal in a single round-trip.

...
## 2026-03-24 - N+1 query loop optimization in Entity Similarity
**Learning:** Performing an N+1 relation query for graph nodes in Python for computing Jaccard Similarity can be extremely slow and cause the event loop to block.
**Action:** Use a pure SQL CTE query with COUNT, GROUP BY and HAVING directly inside the DB to efficiently calculate intersections and prune irrelevant relations before fetching.

...
## 2024-05-25 - Redundant Regex Compilation in Initializers
**Learning:** `re.compile()` calls placed inside `__init__` can cause unnecessary overhead when the class is instantiated frequently, specifically in `CanonScanner` which checks evolutionary changes and may be initialized multiple times inside loop evaluations.
**Action:** Always lift invariant operations like `re.compile()` or defining constants/patterns to the class level to avoid redundant execution during object instantiation.

...
## 2025-03-07 - [Optimize Graph Path Finding with SQL Cycle Pruning]
**Learning:** Adding `AND (',' || p.path_ids || ',') NOT LIKE ('%,' || cast(r.target_id as text) || ',%')` to the `WHERE` clause of a recursive CTE prevents infinite recursion and combinatorial explosion in dense cyclic graphs directly at the database level.
**Action:** When implementing graph traversal in SQL using recursive CTEs, always include a cycle-checking condition using path concatenation directly in the CTE to avoid fetching and processing massive invalid result sets in the Python layer, dramatically improving performance (e.g., ~177x speedup).

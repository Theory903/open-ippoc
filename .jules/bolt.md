...
## 2026-07-08 - Optimize EconomyManager Serialization
**Learning:** High-frequency access paths calling `dataclasses.asdict()` (like `EconomyManager._save` or `snapshot`) suffer large overhead (~25x slower) because `asdict` deep-copies everything, including nested dictionaries and lists.
**Action:** Replaced `asdict()` with manual dictionary construction and shallow `.copy()` for mutable nested fields. Removed unused `asdict` import.

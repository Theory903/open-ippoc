...
## 2024-05-05 - Python Generator Expression Latency
**Learning:** Python generator expressions (`sum(1 for term in terms if term in content)`) introduce significant overhead when called repetitively in tight loops over simple arrays, due to function call stacks per-element.
**Action:** When doing bulk text processing or intent classification routing, explicitly unroll such expressions into raw `for` loops (and pre-compute `len()` or `.lower()`), leading to ~3x performance jumps inside string scanning segments.

...
## 2024-05-30 - Optimize substring matching in Python
**Learning:** Generator expressions with `any()` and lists (e.g., `any(word in query for word in words)`) introduce overhead that makes them ~60% slower than explicit `for` loop iterations combined with tuples for class-level constants when scanning text.
**Action:** Always prefer unrolled `for` loops and class-level tuple constants over generator expressions when checking if multiple substrings exist within a string, particularly in hot paths like query classification.

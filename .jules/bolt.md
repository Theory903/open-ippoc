...
## 2024-05-18 - Replacing dataclasses.asdict with manual dict construction in high-frequency serialization
**Learning:** `dataclasses.asdict` performs a deep copy under the hood, making it extremely slow (~380ms vs ~18ms for 1000 calls on moderately nested structures). In high-frequency operations like the `EconomyManager`'s `snapshot` or `_save` logic, this adds measurable and unnecessary latency.
**Action:** Replace `asdict(instance)` with a custom `.to_dict()` method on dataclasses that manually constructs the dictionary using shallow copies (`.copy()`) for nested data when full deep copies are not required.

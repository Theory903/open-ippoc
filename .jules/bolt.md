...
## 2024-05-24 - Dataclasses asdict Deep Copy Overhead
**Learning:** `dataclasses.asdict` performs a deep copy of the dataclass and all nested structures, which can be surprisingly slow (~5ms per call) in high-frequency serialization paths like `EconomyManager`. Manual dictionary construction with shallow copies (`.copy()`) is up to 600x faster.
**Action:** When optimizing dataclass serialization in performance-critical or high-frequency paths, manually construct the dictionary and use shallow `.copy()` for nested lists/dicts instead of relying on `asdict`.

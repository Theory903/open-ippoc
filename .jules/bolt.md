...
## 2024-05-24 - Unrolled any() loops in intent classification
**Learning:** Using `any()` generator expressions over small inline lists inside hot path string checking is significantly slower than unrolled `for` loops iterating over predefined class-level tuples, reducing performance by 64% in `_classify_intent` in `AdaptiveRAGRouter`.
**Action:** When performing substring or keyword matching on hot paths, avoid generator expressions and use unrolled `for` loops with class-level tuple constants to eliminate iteration and instantiation overhead.

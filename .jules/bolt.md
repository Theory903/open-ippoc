...
## 2024-06-03 - Dataclass Serialization Overhead
**Learning:** Using `dataclasses.asdict` in high-frequency paths (like `EconomyManager._save` and `snapshot` called by orchestrator tick/invocation) adds significant (~5ms) deep-copy overhead.
**Action:** For performance-critical serialization where deep copies aren't strictly necessary, use manual dictionary construction with shallow copies (`list(events)`, `v.copy()`) to reduce latency to <0.1ms.

...
## 2024-05-19 - Entity Similarity Search Optimization
**Learning:** In the knowledge graph `find_similar_entities` method, iteratively querying relations for every other entity resulted in a severe N+1 query problem, taking over 10 seconds for 250 similar entities in benchmarks.
**Action:** Replace the iterative loop with a single SQL query using Common Table Expressions (CTEs) that calculates intersection and Jaccard similarity directly within the database engine. This reduces round-trips to the database and scales effectively, improving performance by over 130x (~0.07s).

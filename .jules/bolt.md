...
## 2026-06-23 - Graph Jaccard Similarity Optimization
**Learning:** Calculating Jaccard similarity between entities in application code by fetching all candidate relationships results in an N+1 query problem, massive network transfer, and high memory overhead for large graphs.
**Action:** When computing graph similarity or aggregations, use SQL Common Table Expressions (CTEs) to calculate intersection and union counts entirely within the database. Filter on intersections first to prune candidates before joining remaining totals, drastically reducing DB-to-application overhead.

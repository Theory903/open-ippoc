...
## $(date +%Y-%m-%d) - Optimize Entity Similarity Search
**Learning:** Python-level entity comparison for Jaccard similarity causes severe N+1 query problems and latency spikes (~1s) when scanning graphs with even just 1000 entities, due to individual query overhead per entity comparison.
**Action:** Replaced iterative N+1 Python logic with a single bulk SQL query using Common Table Expressions (CTEs) that computes candidate intersections and Jaccard similarity entirely within the database engine, improving performance significantly.

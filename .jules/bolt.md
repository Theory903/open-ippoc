...
## 2024-06-11 - dataclasses.asdict Performance Overhead
**Learning:** Using `dataclasses.asdict` on deeply nested structures inside high-frequency operations (like `_save` and `snapshot` in `EconomyManager`) introduces severe performance overhead (~5ms per call) due to deep-copying behavior.
**Action:** Avoid `dataclasses.asdict` in high-frequency paths. Manually construct dictionaries and use shallow `.copy()` for nested structures, which is significantly faster.

...
## 2024-05-24 - String Check Generators
**Learning:** Generator expressions with substring matching (e.g., `any(word in query for word in words)`) add substantial overhead compared to explicit unrolled for-loops. Caching word lists into class-level tuples rather than instantiating lists per call further reduced check latencies by over 2.6x.
**Action:** When implementing high-frequency intent classification or routing, hoist invariant keyword lists to class-level tuples and unroll loop logic explicitly to avoid generator instantiation costs.

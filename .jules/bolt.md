...
## 2024-05-24 - Dataclasses.asdict Overhead
**Learning:** `dataclasses.asdict` introduces significant overhead (up to ~5ms per call) when used in tight, high-frequency paths like `EconomyManager` operations during tool invocations, due to its deep-copy semantics and reflection.
**Action:** For performance-critical code paths that serialize small dataclasses frequently, replace `asdict()` with manual dictionary construction to drastically reduce CPU time.

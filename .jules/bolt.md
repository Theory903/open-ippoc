...
## 2024-06-27 - Entity Similarity SQL JOIN Optimization
**Learning:** Performing iterative queries on N entities to retrieve relations and manually calculating Jaccard similarity in Python causes a classic N+1 query problem, taking ~350ms for a dense graph of 200 nodes.
**Action:** Offload the similarity calculation and neighbor matching completely to a SQL JOIN with Common Table Expressions (CTEs), reducing round-trips and dropping query time to ~8ms.

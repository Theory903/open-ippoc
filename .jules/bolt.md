...
## 2024-06-15 - Dataclass Serialization Overhead in High-Frequency Paths
**Learning:** Using `dataclasses.asdict` for deep serialization of complex nested state objects (like `EconomyState`) adds significant (~5-7ms) overhead per call due to its recursive deep-copy mechanism. In high-frequency paths like an economy tick or tool orchestration, this micro-latency compounds quickly.
**Action:** For performance-critical state serialization, replace `asdict` with manual dictionary construction and shallow `.copy()` for nested collections. This approach is 15-20x faster.

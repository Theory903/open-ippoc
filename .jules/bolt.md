...
## 2024-05-22 - [dataclasses.asdict Overhead]
**Learning:** `dataclasses.asdict` does deep copies by default, causing massive ~5ms/call overhead in high-frequency serialization paths like `EconomyManager` state saves.
**Action:** Replace `asdict` with manual dictionary construction and shallow `.copy()` for nested structures in performance-critical paths.

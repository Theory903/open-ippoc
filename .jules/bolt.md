...
## 2024-05-24 - Optimizing Python Generator Expressions in Hot Paths
**Learning:** Instantiating lists locally and using `any()` or `sum()` with generator expressions inside hot-path classification heuristics (like RAG routing) adds significant redundant object creation overhead.
**Action:** Hoist static keyword lists to class-level tuple/set constants and use unrolled `for` loops or set intersections for multi-keyword substring matching.

...
## 2025-02-21 - Optimize dataclass serialization in EconomyManager
**Learning:** `dataclasses.asdict` has high overhead (~5ms/call depending on size) because it deeply copies all structures. In high-frequency paths like `EconomyManager._save()` and `snapshot()`, this creates an unnecessary bottleneck.
**Action:** When optimizing dataclass serialization in performance-critical paths, manually construct dictionaries and use shallow `.copy()` for nested structures instead of relying on `asdict()`.

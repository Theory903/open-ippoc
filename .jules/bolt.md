...
## 2025-04-23 - Optimize RAG router query classification string matching overhead
**Learning:** Generator expressions inside any() and sum() calls with local inline string arrays incur significant overhead due to per-call list allocation and generator instantiation overhead when evaluated repeatedly (e.g., query.lower()).
**Action:** Always unroll string matching generator expressions into explicit for-loops, hoist invariant operations like lower() to the method top level, and replace inline arrays with static class-level tuple constants to bypass instantiation latency.

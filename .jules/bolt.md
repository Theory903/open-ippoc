...
## 2024-05-19 - [Graph path finding performance lookup]
**Learning:** In Mnemosyne's `graph/manager.py`, iterating over large sets of node IDs and calling `id_to_name.get(nid)` with an `if name is None:` check introduces significant overhead.
**Action:** Replace manual loops and `.get()` calls with list comprehensions and direct dictionary indexing wrapped in a `try...except KeyError` block for large lookups to leverage C-speed execution and the EAFP pattern, which is ~15% faster.

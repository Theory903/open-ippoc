...
## 2024-05-24 - RAG Router Intent Classification Overhead
**Learning:** Using inline lists and generator comprehensions inside `any()` expressions (e.g. `any(word in query for word in ['a', 'b'])`) during high-frequency routing introduces measurable overhead (~60% slower) due to runtime allocation and generator function calls.
**Action:** When evaluating substrings against static collections in hot loops or routers, extract lists to class-level tuple constants and replace `any()` with explicitly unrolled `for` loops utilizing early breaks.

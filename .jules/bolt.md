...
## 2024-05-24 - SQL Recursive CTE Cycle Detection
**Learning:** While pushing graph traversals to recursive SQL CTEs eliminates N+1 query latency, CTEs inherently lack the cycle-prevention logic typical of procedural BFS/DFS, which can lead to exponential row explosions in cyclic graphs. Processing cycles in Python *after* the query returns is too late.
**Action:** Prevent runaway CTE recursion by accumulating visited path IDs as strings in the CTE itself and conditionally joining (`AND ',' || p.path_ids || ',' NOT LIKE '%,' || cast(r.target_id as text) || ',%'`) to halt traversal when a cycle is detected during execution.

...
## 2024-05-15 - [Generator Expressions in Routing]
**Learning:** Using `any()` with a generator expression inside frequent intent classification (e.g., `any(word in query for word in words)`) incurs significant dynamic allocation and overhead compared to an explicit unrolled loop with tuple constants, resulting in a ~2.5x slowdown.
**Action:** When evaluating intent keyword lists in high-throughput routers, hoist the immutable lists to class-level tuples and unroll the substring matching into explicit `for` loops.

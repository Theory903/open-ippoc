...
## 2024-05-18 - RAG Router generator expression optimization
**Learning:** In routing functions, generator expressions (`any(word in query for word in words)`) create significant overhead on high-frequency paths compared to unrolled `for` loops and class-level tuple constants.
**Action:** Replaced generator expressions with explicit `for` loops and hoisted static lists to class-level tuples for ~2x performance boost.

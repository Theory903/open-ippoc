...
## 2025-02-28 - CanonScanner Optimization
**Learning:** Compiling regex patterns repeatedly inside class constructors causes significant CPU overhead, especially when instantiated frequently in validation loops.
**Action:** Hoist `re.compile` calls to class-level attributes for static patterns to parse them exactly once upon module load.

...
## 2026-05-29 - N+1 Query in Entity Similarity Search
**Learning:** Using an N+1 query to calculate Jaccard similarity between graph entities is highly inefficient and creates significant latency, even for small numbers of entities.
**Action:** Implement Intersection-first optimization using Common Table Expressions (CTEs) to offload the similarity calculation completely to the database, ensuring query performance remains fast as entity counts scale.

...
## 2024-06-08 - Avoid dataclasses.asdict in high-frequency nested serialization
**Learning:** In `EconomyManager`, `dataclasses.asdict` was used to snapshot state for disk writes. Due to deep-copying, it was found to be exceptionally slow (~350x slower) compared to manually building dictionaries and shallow-copying (`.copy()`) inner collections.
**Action:** Use manual dictionary construction instead of `asdict()` for frequent state snapshots, especially for nested structures in performance-critical paths.

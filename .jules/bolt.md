...
## 2024-06-28 - Optimized N+1 Query in Graph Path Finding
**Learning:** High-frequency operations like graph traversal (e.g. BFS) utilizing repeated database queries inside a loop significantly degraded performance due to N+1 query patterns.
**Action:** Replaced iterative application-side graph traversal logic with Recursive Common Table Expressions (CTEs) when interacting with relational stores (SQLite/PostgreSQL) to push traversal logic entirely to the database engine and enable bulk-fetching entity names.

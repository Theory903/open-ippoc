...
## 2024-04-24 - N+1 Query in Entity Similarity Search Optimization
**Learning:** Using iterative Python loops to compute graph metrics (like Jaccard similarity) over individual relations creates massive N+1 query bottlenecks in `GraphManager`.
**Action:** Replace manual loop/set intersection logic with an optimized SQL Common Table Expression (CTE) query. Use techniques like `HAVING COUNT(r.id) >= :ref_total * :threshold` to mathematically prune candidates early (since `Union >= ref_total`, `Intersection / ref_total >= Intersection / Union`), offloading the work to the database and achieving multi-order-of-magnitude speedups.

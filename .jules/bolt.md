...
## 2026-06-03 - [Optimize graph entity similarity search]
**Learning:** Checking similarity between nodes in a graph structure using application-side N+1 queries is extremely inefficient compared to single-query Common Table Expressions (CTEs).
**Action:** Always prefer CTE-based bulk fetching, intersection, and aggregation directly within SQL for complex relational queries rather than performing iterative O(N) queries across all entities in Python.

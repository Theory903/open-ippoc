...
## 2024-05-28 - Optimize dataclasses.asdict in core paths
**Learning:** `dataclasses.asdict` is known to have significant deep-copy overhead (~5ms/call) which becomes a bottleneck in high-frequency paths like `EconomyManager` (saving state and tracking tools) and `InMemoryLedger` (tracking executions). It turns out manually creating dictionaries and using shallow `.copy()` is significantly faster.
**Action:** Replace `dataclasses.asdict` with manual dictionary construction in `EconomyManager` and `InMemoryLedger` for serialization. Avoid `dataclasses.asdict` in high-frequency execution paths in the future.

...
## 2026-05-29 - Entity Similarity Search Optimization
**Learning:** Using iterative O(N) looping with N+1 queries to compare knowledge graph entities for shared relationships results in significant overhead, especially as the database grows, taking ~8s for 1k entities.
**Action:** Rely on SQL CTEs and set-based intersection-first approaches directly inside the database query to drastically reduce data serialization and Python loop overhead. This drops execution to ~40ms.

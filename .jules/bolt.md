...
## 2026-03-01 - CTE instead of N+1 for graph similarity
**Learning:** N+1 queries in graph relation similarity functions cause severe scaling bottlenecks due to loop-based query execution for each entity in python.
**Action:** Shift complex iterative set operations (like intersections and unions for Jaccard similarity) directly into SQL using Common Table Expressions (CTEs) when using SQLAlchemy to significantly reduce database roundtrips and execution time.

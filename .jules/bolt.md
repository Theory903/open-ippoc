...
## 2024-07-04 - Optimize dataclass serialization in high-frequency paths
**Learning:** `dataclasses.asdict` does a deep copy by default, which introduces significant overhead (~5ms/call for complex objects) when used in high-frequency paths like `EconomyManager._save()` and `EconomyManager.snapshot()`.
**Action:** When optimizing Python dataclass serialization in high-frequency paths, manually construct dictionaries and use shallow `.copy()` for nested structures to avoid `asdict()` deep-copy overhead.

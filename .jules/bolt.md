...
## 2024-03-27 - Hoist Invariant Routing Operations
**Learning:** In RAG routing and retrieval components (`AdaptiveRAGRouter`, `SemanticManager`), invariant operations such as `.lower()`, `.split()`, regular expressions, and `set()` creations must be optimized. Recomputing these inside hot paths degraded throughput.
**Action:** Hoist these invariant operations outside iteration loops over large datasets, define static keyword lists as class-level or module-level constants, and use `re.compile()` at the module level to prevent redundant overhead.

...
## 2024-10-24 - Dataclass Serialization Overhead
**Learning:** `dataclasses.asdict` performs a deep copy recursively, which introduces significant overhead (~5ms per call) in high-frequency paths like `EconomyManager` that save or snapshot state frequently.
**Action:** Replace `asdict` with manual dictionary construction and shallow copies (e.g., `.copy()`) for nested structures when performance is critical.

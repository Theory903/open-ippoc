...
## 2024-10-24 - Dataclass Serialization Overhead in High-Frequency Paths
**Learning:** Using `dataclasses.asdict` in high-frequency paths like `EconomyManager` for deep-copying state introduces significant serialization overhead (e.g. ~7ms vs ~0.3ms per 10k items) compared to manual dictionary mapping.
**Action:** When optimizing dataclass serialization in performance-critical areas, manually construct dictionaries and use shallow `.copy()` for nested structures to avoid deep-copy overhead.

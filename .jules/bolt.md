...
## 2024-05-24 - N+1 Query in Entity Similarity Search
**Learning:** Finding similar entities based on graph relationships by running individual queries for every entity is incredibly slow (20s for 1000 unrelated entities). Computing the Jaccard similarity directly in the database using CTEs (Common Table Expressions) and `JOIN`s reduces the time drastically (0.02s).
**Action:** When calculating similarity or intersections across many database entities, always push the logic to the database (using CTEs or grouping) rather than fetching all rows and doing nested Python loops.

...
## 2024-06-07 - Dataclasses asdict overhead in high frequency paths
**Learning:** Using `dataclasses.asdict` to serialize state in high-frequency methods (like `EconomyManager.snapshot` and `_save`) causes severe performance bottlenecks due to its deep-copy mechanism.
**Action:** When serializing simple data classes, manually construct the dictionary and use `.copy()` for shallow nested structures to significantly improve performance.

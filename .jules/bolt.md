...
## 2024-05-25 - Unrolled Loops over Generators in Router
**Learning:** Using generator expressions with `any()` and list comprehensions with `sum()` for substring matching (e.g., `any(q in query.lower() for q in indicators)`) is significantly slower (almost 4x) than evaluating `query.lower()` once and using explicit unrolled `for` loops. Hoisting static lists out of method scope to class-level constants also eliminates redundant memory allocations.
**Action:** When performing high-frequency text classification or routing, hoist invariant operations (like `.lower()`) and static keyword lists outside loops/methods, and prefer explicit unrolled `for` loops over Pythonic generator expressions for maximum speed.

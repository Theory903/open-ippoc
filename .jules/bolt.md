...
## 2024-05-18 - Optimized N+1 Query in Graph Manager Entity Search
**Learning:** Finding similar entities by pulling relationships into Python and iterating over all entities in the database (N+1 queries) creates a massive performance bottleneck, increasing latency to ~10s for 10k entities.
**Action:** When performing Jaccard similarity or intersection logic across many graph nodes, push the intersection computation into SQL using CTEs. This avoids full table scans of unrelated entities and reduces latency to ~10ms.

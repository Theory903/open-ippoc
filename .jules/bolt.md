...
## 2024-06-25 - dataclasses.asdict Overhead
**Learning:** Using `dataclasses.asdict` in high-frequency paths like `EconomyManager` adds significant overhead (~5ms per call) because it performs a deep copy of nested structures by default. This makes `_save`, `update_tool_stats`, and `snapshot` operations block the main thread longer than necessary.
**Action:** For high-frequency serialization, construct dictionaries manually and use shallow `.copy()` for nested lists/dicts to bypass the recursive deep-copy overhead of `asdict`.

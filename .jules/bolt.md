...
## 2026-06-25 - Recursive CTE Optimization over Application-level BFS
**Learning:** In highly-connected graph structures queried via SQL databases, using a recursive Common Table Expression (CTE) to retrieve relationship paths drastically outperforms application-level breadth-first search logic. Application-level loops cause N+1 query patterns.
**Action:** Whenever implementing pathfinding logic, push the recursive query generation down to the database via CTEs, and perform bulk entity resolution to eliminate N+1 latency.

...
## 2026-05-30 - Dataclass Serialization Overhead in EconomyManager
**Learning:** Using `dataclasses.asdict` on complex nested structures in high-frequency paths (like `EconomyManager.tick` or `snapshot`) incurs significant deep-copy overhead (~1.9ms per 1000 items vs ~0.09ms manual).
**Action:** Manually construct dictionaries and use shallow `.copy()` for nested list/dict structures when serializing dataclasses in performance-critical paths.

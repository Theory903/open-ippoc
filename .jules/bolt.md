...
## 2024-05-18 - N+1 Query in Entity Similarity Search
**Learning:** Found N+1 query loop when comparing entity relationships in `GraphManager.find_similar_entities`.
**Action:** Replace the loop with a Common Table Expression (CTE) query that finds all similar entities in a single database request.

...
## 2026-03-20 - optimize Jaccard similarity candidate pruning in graph manager
**Learning:** Calculating Jaccard similarity using multiple nested database queries (an N+1 query problem) where relations for each candidate entity are queried separately scales poorly and consumes excessive memory and CPU.
**Action:** Replace nested loops performing individual relationship queries with an intersection-first optimization using SQL Common Table Expressions (CTEs). By calculating the Jaccard similarity entirely within the database with a single query, and dynamically utilizing `HAVING COUNT` for threshold pruning, we significantly reduce data transferred over the network and latency caused by multiple round-trips. Always ensure CTE logic mirrors the original behavior (e.g., using `COUNT(DISTINCT r.id)` if handling unique relationships like Python's `set()`).

...
## 2026-03-17 - Invariant String Operations Hoisting in RAG Retrievals
**Learning:** Found a severe performance bottleneck in `_embedding_similarity_search` inside `src/ippoc/mnemosyne/semantic/rag.py` and its infra counterpart where `_extract_semantic_components` (which relies heavily on regexes) was being called on an invariant `query_embedding` string inside a loop that iterated over all documents (`all_docs`). Additionally, set operations were being recalculated inside the loop using the invariant output.
**Action:** Always verify loops over large datasets (like `all_docs`) and explicitly hoist invariant string operations, expensive computations, and `set` creations outside the loop to execute them exactly once. In Python, this changes an O(N) regex and set allocation overhead into O(1) before the loop runs.

...
## 2024-05-24 - Async File I/O in AutonomyController Synchronous Methods
**Learning:** `AutonomyController` was performing synchronous file I/O (open/write/close) in `_save_state` and `_record_explain` during every evaluation of intents, which introduced latency and potential blocking on the main execution loop (`run_autonomy_loop`).
**Action:** Offload disk write operations to a background thread (`ThreadPoolExecutor`) after snapshotting the internal dictionary, mimicking the `get_economy()` non-blocking approach to maintain an unblocked main path.

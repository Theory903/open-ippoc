...
## 2024-05-23 - Dataclass Serialization Overhead in Hot Paths
**Learning:** Using `dataclasses.asdict()` in high-frequency paths like `EconomyManager` or `ToolOrchestrator` ledger incurs a heavy deep-copy overhead (~5ms/call for complex structs).
**Action:** When serializing simple dataclasses in hot execution paths or background write queues, manually construct dictionaries and use shallow `.copy()` for nested structures to achieve near 200x speedup over `asdict()`.

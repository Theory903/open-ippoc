...
## 2024-05-19 - GraphManager similarity search performance improvement

**Learning:** The `find_similar_entities` logic in `src/ippoc/mnemosyne/graph/manager.py` had an N+1 query issue, computing Jaccard similarity by looping over all entities and making a database call for each to get their relations. This approach didn't scale well with the database size.

**Action:** It was rewritten to calculate the Jaccard similarity entirely inside a single SQL query using Common Table Expressions (CTEs). The CTE filters candidates that share at least one relationship with the reference entity, thus dramatically improving the performance. Replaced looping overhead with an optimized intersection-first CTE SQL statement. Next time, always prefer calculating similarity measures entirely within the database queries rather than doing full-table scans.

...
## 2024-05-18 - Prevent Event Loop Blocking in Autonomy Controller
**Learning:** Synchronous file I/O operations (`open`, `json.dump`) inside async loops (like `run_cycle` in `autonomy.py`) can block the main event loop thread for extended periods, especially when dealing with large objects like state arrays.
**Action:** Always wrap heavy synchronous file operations within `asyncio.to_thread` and `await` them in async methods to maintain responsiveness in high-throughput components.

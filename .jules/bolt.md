...
## 2024-05-24 - Hoisting Invariant String Operations
**Learning:** In string-heavy RAG retrieval pipelines, operations like `query.lower().split()` inside large loops (e.g., cross-encoder reranking over thousands of documents) cause severe performance degradation by redundantly allocating new string/list objects per iteration.
**Action:** Always hoist invariant operations—such as `.lower()`, `.split()`, or regular expressions—outside of iteration loops over large datasets or collections.

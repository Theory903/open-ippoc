...
## 2024-05-25 - CanonScanner regex compilation overhead
**Learning:** Compiling regular expressions in the `__init__` method of `CanonScanner` caused redundant compilation overhead when multiple instances were created or multiple files were scanned.
**Action:** Compile regular expressions once as a class-level attribute (`_FORBIDDEN_PATTERNS`) to avoid unnecessary overhead.

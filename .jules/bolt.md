...
## 2024-06-21 - dataclasses.asdict deep-copy overhead
**Learning:** Using `dataclasses.asdict` in high-frequency paths (like `EconomyManager.spend`) introduces significant latency (~5ms overhead per call) because it performs deep copies.
**Action:** Avoid `dataclasses.asdict` for dataclass serialization in performance-critical code. Manually construct dictionaries and use shallow `.copy()` for nested structures to drastically improve performance.

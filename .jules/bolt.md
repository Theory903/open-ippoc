...
## 2025-01-20 - [Optimize generator expressions in RAG router]
**Learning:** Hoisting invariant lists to class-level tuple constants and unrolling generator expressions (`any(x in y for x in z)`) into explicit `for` loops significantly reduces computational overhead for high-frequency operations.
**Action:** Use static tuples and unrolled loops in critical fast paths instead of dynamic lists and generators.

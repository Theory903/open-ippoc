...
## 2025-02-28 - Optimized Reference Entity Lookup in find_similar_entities
**Learning:** In `find_similar_entities`, running two separate sequential queries to fetch a reference entity's ID and its total relation count introduces an unnecessary database roundtrip (a 1+1 query pattern).
**Action:** Always combine related lookups into a single query using `LEFT JOIN` and aggregate functions (e.g., `COUNT()`) grouped by the entity ID to minimize database I/O latency.

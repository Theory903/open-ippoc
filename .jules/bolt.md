...
## 2024-05-25 - Python Generator Expression Overhead in String Matching
**Learning:** Using generator expressions within `any()` or `sum()` for basic substring matching (e.g., `any(w in q.lower() for w in words)`) causes significant overhead because it creates a new generator object on every call, and more importantly, it may evaluate methods like `.lower()` repeatedly depending on the execution context. In high-frequency paths like a router's query classification, this overhead accumulates noticeably.
**Action:** Unroll such expressions into explicit `for` loops, hoist string operations like `.lower()` outside the loop, and move the static keyword lists to class-level immutable tuples to avoid reallocation overhead.

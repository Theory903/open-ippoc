...
**Learning:** Calculating Jaccard similarity in Python via N+1 queries (`SELECT e.name, r.relation... WHERE r.source_id = :entity_id` inside a loop over all entities) causes massive performance degradation due to extensive I/O and latency overhead.
**Action:** Push intersection-first pruning and count queries directly into SQL using CTEs, fetching only candidates that share at least the minimum required relations to satisfy the threshold.

...
**Learning:** Using `dataclasses.asdict()` for frequent serialization (e.g., in `EconomyManager`) introduces significant deep-copy overhead (~5ms per call), which degrades performance when called repeatedly in loops or tight logic.
**Action:** Avoid `dataclasses.asdict()` in high-frequency execution paths. Use manual `to_dict()` mapping methods and shallow `.copy()` for nested structures to eliminate deep-copy overhead.

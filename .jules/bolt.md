...
## 2026-02-28 - Optimize N+1 Query in Entity Similarity Search
**Learning:** In `find_similar_entities`, looking up relationships for every single unrelated entity in the database created a massive N+1 query bottleneck (taking ~4.3s for 500 entities). Refactoring the logic into a single SQL CTE to compute intersection sizes natively in the database bypassed Python loop overhead and irrelevant entity scans, reducing lookup time to ~0.04s.
**Action:** When computing graph similarity or traversing relationships, avoid N+1 queries by pushing grouping, intersection, and filtering logic into SQL CTEs instead of iterating and fetching relationships row-by-row in Python.

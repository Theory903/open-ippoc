...
## 2026-05-29 - SQL Jaccard Similarity Optimization
**Learning:** Moving set operations (like Jaccard similarity calculation) and filtering out of application loops (N+1 queries) and into the database using Common Table Expressions (CTEs) can result in massive performance improvements (76x speedup) by avoiding network overhead and full table scans.
**Action:** Always prefer SQL CTEs and JOINs for graph traversal and relationship filtering rather than fetching entity lists and resolving edges iteratively in Python.

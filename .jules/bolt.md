...
## 2024-05-28 - [Avoid dataclasses.asdict for performance-critical code]
**Learning:** `dataclasses.asdict` performs deep copies which introduces significant overhead (~5-10x slower) in high-frequency serialization paths (like `EconomyManager.snapshot` or `save`).
**Action:** When optimizing Python dataclass serialization in high-frequency paths, avoid `dataclasses.asdict`. Instead, manually construct dictionaries and use shallow `.copy()` for nested structures to achieve ~10x performance improvement.

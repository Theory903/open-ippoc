...
## 2024-05-18 - Recursive CTE over iterative BFS for Graph paths
**Learning:** Using Python loops with N+1 queries for deep BFS graph traversals (`manager.find_relationship_path`) is extremely slow (~0.1305s).
**Action:** Replace iterative BFS with SQL Recursive CTEs where supported to push the path finding logic down to the database and eliminate Python network roundtrips (~0.0531s). Ensure strings/casts are portable between SQLite/Postgres.

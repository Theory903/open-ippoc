...
## 2024-05-19 - Fast string suffix matching
**Learning:** For string suffix/prefix matching against multiple options in Python, passing a tuple directly to `str.endswith()` or `str.startswith()` (e.g., `string.endswith(('.png', '.jpg'))`) utilizes optimized C-level implementations and is significantly faster than using a generator expression with `any()` and a set.
**Action:** Use tuple passing for `endswith` and `startswith` instead of iterating over extensions with `any()`.

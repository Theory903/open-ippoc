...
**Learning:** `dataclasses.asdict` imposes significant deep-copy overhead (~700ms per 1000 calls) in high-frequency paths like `EconomyManager` serialization, which blocks operations longer than necessary.
**Action:** When serializing simple, frequently-accessed dataclasses, manually construct dictionaries and use shallow `.copy()` for nested collections to improve performance by >100x.

...
## 2024-05-28 - String Invariant Hoisting and Regex Compilation
**Learning:** In RAG routing and retrieval components (e.g., `AdaptiveRAGRouter` and `SemanticManager`), invariant string operations such as `.lower()`, `.split()`, and regular expressions inside `.findall()` or `.split()` can become performance bottlenecks when executed inside loops, generator expressions, or repeated deeply within evaluation methods. These redundant operations allocate strings and compile regex patterns dynamically on every pass, generating significant overhead.
**Action:** Always pre-compile regexes at the class level and hoist invariant string operations (`query.lower()`) or set computations (`set(query_components)`) outside iteration loops to prevent O(N*M) allocation overhead.

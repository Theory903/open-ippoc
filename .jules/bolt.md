...
## 2025-05-18 - Optimized dataclasses.asdict serialization
**Learning:** `dataclasses.asdict` performs deep copies and type checks making it extremely slow (~500x slower) when heavily used in hot paths like the `_save` and `snapshot` functions of `EconomyManager` (e.g. 13ms vs 0.02ms).
**Action:** Replace `asdict` with a manual dictionary mapping method (`_state_to_dict`) that builds the dictionary explicitly and uses shallow copies (`.copy()`) where appropriate to avoid mutations.

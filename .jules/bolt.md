...
## 2024-06-23 - Avoid dataclasses.asdict in High-Frequency Paths
**Learning:** dataclasses.asdict has significant deep-copy overhead (~2.3ms vs ~0.003ms for manual construction), causing serialization bottlenecks in high-frequency loops like those in EconomyManager and AutonomyController.
**Action:** When serializing simple or flat dataclasses in performance-critical paths, manually construct dictionaries instead of using asdict. Use shallow .copy() for nested dicts/lists if needed.

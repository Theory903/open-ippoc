...
## 2024-05-24 - Python dataclasses.asdict Overhead
**Learning:** Using `dataclasses.asdict` inside high-frequency loops and core paths (like `EconomyManager.tick` via `spend`/`save` or `InMemoryLedger` read ops) introduced significant serialization overhead (~5-10ms per batch operation) due to its recursive deep-copying behavior.
**Action:** Replace `dataclasses.asdict()` with shallow `.__dict__.copy()` or manual dictionary mappings for immutable/simple properties in high-throughput critical path serialization.

...
## 2026-05-20 - [Graph CTE Optimization]
**Learning:** Replaced a slow O(N) Python loop querying relationships for all non-reference entities with an efficient O(Neighbors) Recursive CTE in SQL for Jaccard similarity.
**Action:** When filtering or comparing against a large dataset across relations, prefer SQL-side aggregations and CTEs to prune candidates before loading data into Python.

...
## 2024-06-04 - [Avoid dataclasses.asdict in high-frequency paths]
**Learning:** `dataclasses.asdict` performs a deep copy under the hood, which creates massive overhead (~5ms/call) in high-frequency serialization paths (like `EconomyManager` snapshots) when nested structures like `events` grow large.
**Action:** Manually construct dictionaries and use shallow `.copy()` for nested collections in performance-critical serialization paths instead of relying on `asdict`.

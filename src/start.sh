...
node runtime/bootstrap/genesis.ts generate

# 3. WAKE ORGANS (Services)
# Start strictly in order: Memory -> Body -> Brain -> Kernel Bridge
echo "🧠 Waking Mnemosyne (Memory)..."
docker compose -f runtime/supervisor/organism.yaml up -d mnemosyne
...
docker compose -f runtime/supervisor/organism.yaml up -d soma
...
docker compose -f runtime/supervisor/organism.yaml up -d cortex
...
docker compose -f runtime/supervisor/organism.yaml up -d gateway

# 5. ACTIVATE WATCHDOG
echo "🐕 Activating Watchdog monitor..."
python3 runtime/supervisor/watchdog.py &

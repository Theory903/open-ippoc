import { getIPPOCAdapter } from "../ippoc-adapter.js";
export class Thalamus {
    ...
    initializeReflexRules() {
        return [
                ...
                action: (signal) => {
                    ...
                    try {
                        ...
                        import("child_process").then(cp => {
                            // Security Fix: Use execFile instead of exec to prevent command injection via pid
                            cp.execFile("renice", ["+10", "-p", String(signal.payload.pid)], (error) => {
                                if (error) {
                                    console.error("Failed to renice process:", error);
                                }
                            });
                        });
            ...
            {
                condition: (signal) => signal.type === "HIVE_PREFIX" &&

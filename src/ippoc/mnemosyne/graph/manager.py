...
class GraphManager:
    ...
    async def find_similar_entities(
        ...
        try:
            async with self.Session() as session:
                # 1. Fetch reference entity details in a single query to initialize the search
                ref_stmt = text("""
                    SELECT e.id, COUNT(r.id) as total_relations
                    FROM kg_entities e
                ...
                if not ref_row or not ref_row.id or ref_row.total_relations == 0:
                    return []

                ref_id = ref_row.id
                ref_total = ref_row.total_relations

                # 2. Optimized CTE-based bulk query replacing the N+1 `for` loop logic.
                # Instead of fetching all entities and then querying relations for each in Python,
                # we calculate intersection (shared edges) and total degrees directly in SQL.
                bulk_similarity_stmt = text("""
                    WITH ref_edges AS (
                    ...
                    candidate_intersections AS (
                        ...
                        FROM kg_relations r
                        JOIN ref_edges re ON r.target_id = re.target_id AND r.relation = re.relation
                    ...
                    candidate_totals AS (
                        ...
                        FROM kg_relations r
                        JOIN candidate_intersections ci ON r.source_id = ci.source_id
                        GROUP BY r.source_id
                    )
                    SELECT
                        e.name as entity_name,
                        ci.shared_count,
                        (CAST(ci.shared_count AS FLOAT) / (ct.total_count + :ref_total - ci.shared_count)) as similarity_score
                    FROM candidate_intersections ci
                    JOIN candidate_totals ct ON ci.source_id = ct.source_id
                    JOIN kg_entities e ON ci.source_id = e.id
                    WHERE (CAST(ci.shared_count AS FLOAT) / (ct.total_count + :ref_total - ci.shared_count)) >= :threshold
                    ORDER BY similarity_score DESC
                """)

                results = await session.execute(
                    bulk_similarity_stmt,
                    {
                        "ref_id": ref_id,
                        "ref_total": ref_total,
                ...
                return [
                    {
                        ...
                        "shared_relations": row.shared_count,
                    }

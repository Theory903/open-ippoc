...
class GraphManager:
    ...
    async def get_entity_context(self, entity_name: str, context_types: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            async with self.Session() as session:
                # Get entity details
                # Fixed: Use 'metadata' column name instead of 'metadata_' to match DB schema
                entity_stmt = text("""
    ...
    async def find_similar_entities(self, entity_name: str, similarity_threshold: float = 0.7) -> List[Dict[str, Any]]:
        ...
        try:
            async with self.Session() as session:
                ...
                # 1. ref_info: Get ID and total relations count for reference entity
                # 2. ref_rels: Get outgoing relations of reference entity
                # 3. candidates: Count intersections with other entities
                # 4. candidate_totals: Count total relations for candidates
                # 5. Main SELECT: Calculate Jaccard similarity
                stmt = text("""
                    WITH ref_info AS (

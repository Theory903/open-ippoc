    ...
    async def find_similar_entities(
        ...
        try:
            async with self.Session() as session:
                ...
                # Optimization: Intersection-first optimization using CTEs
                ...
                # Note: This query avoids full table scans of unrelated entities and N+1 queries.
                stmt = text("""
                    WITH ref_rels AS (
                    ...
                    candidates AS (
                        ...
                        HAVING CAST(COUNT(r.id) AS FLOAT) >= :ref_total * :threshold
                    ),
                    candidate_totals AS (
                        SELECT
                            r.source_id,

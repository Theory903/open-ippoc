...
class GraphManager:
    ...
    async def _find_paths_cte(
        ...
        # Optimization: We replace the Python-level BFS traversal, which suffered from the N+1 queries problem
        # (doing a separate database query to find edges for every single node visited),
        # with a single recursive CTE (`WITH RECURSIVE`). The database natively computes the graph paths
        # much more efficiently. We then extract all referenced IDs and perform exactly one
        # bulk fetch query to map those IDs back to entity names, bringing the query count from O(N) to exactly 2.

...
def issue_api_key(request: Request, node_id: str = "ippoc-local"):
    # Restrict token issuance to local loopback to prevent unauthorized remote token generation
    allowed_hosts = {"127.0.0.1", "::1", "localhost", "testserver"}
    client_host = request.client.host if request.client else None
    if client_host not in allowed_hosts:
        raise HTTPException(status_code=403, detail="Forbidden: API key issuance restricted to local interfaces")

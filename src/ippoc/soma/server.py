...
def issue_api_key(request: Request, node_id: str = "ippoc-local"):
    # Security: Prevent unauthorized remote token generation by restricting API key issuance to local loopback
    if request.client and request.client.host not in ("127.0.0.1", "::1", "localhost", "testclient"):
        raise HTTPException(status_code=403, detail="API key issuance restricted to local loopback interface")

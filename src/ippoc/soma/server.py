...
def issue_api_key(request: Request, node_id: str = "ippoc-local"):
    if not request.client or request.client.host not in ("127.0.0.1", "::1", "localhost", "testserver"):
        raise HTTPException(status_code=403, detail="API key issuance is restricted to local loopback interface")

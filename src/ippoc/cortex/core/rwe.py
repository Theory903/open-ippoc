...
import time
from ippoc.cortex.core.economy import EconomyManager, get_economy
...
class ReputationWeightedEconomy(EconomyManager):
                ...
                with open(trust_path, "r") as f:
                    ...
                    for peer_id, metrics_dict in data.items():
                        self.trust_metrics[peer_id] = TrustMetrics(
                            ...
                            historical_alignment=metrics_dict.get(
                                "historical_alignment", 0.5
                            ),
                            interaction_count=metrics_dict.get("interaction_count", 0),
                            last_interaction=metrics_dict.get(
                                "last_interaction", time.time()
                            ),
                            value_contributed=metrics_dict.get(
                                "value_contributed", 0.0
                            ),
    ...
    def _save_trust_metrics_to_disk(self, data: dict, trust_path: str) -> None:
        try:
            import json
            import os

            os.makedirs(os.path.dirname(trust_path), exist_ok=True)
            temp_path = trust_path + ".tmp"
            with open(temp_path, "w") as f:
                json.dump(data, f, indent=2)
            os.replace(temp_path, trust_path)
        except Exception:
    ...
    def _save_trust_metrics(self) -> None:
        ...
        if not hasattr(self, "_trust_executor"):
            from concurrent.futures import ThreadPoolExecutor

            self._trust_executor = ThreadPoolExecutor(
                max_workers=1, thread_name_prefix="trust_writer"
            )
        self._trust_executor.submit(self._save_trust_metrics_to_disk, data, trust_path)
    ...
    def get_peer_trust(self, peer_id: str) -> TrustMetrics:
        ...
        if peer_id not in self.trust_metrics:
            ...
            rep_score = (
                self.reputation_engine.get_reputation(peer_id)
    ...
    def get_effective_budget(
        self, peer_id: Optional[str] = None, base_priority: float = 1.0
    ) -> float:
        """
    ...
    def allocate_budget(
        self, peer_id: str, requested_amount: float, purpose: str, priority: float = 0.5
    ) -> Dict[str, Any]:
        """
    ...
    def record_peer_contribution(
        self, peer_id: str, value: float, success: bool, context: Dict[str, Any] = None
    ) -> None:
        """
            ...
            rankings.append(
                {
                    ...
                    "interaction_count": metrics.interaction_count,
                    "net_impact": metrics.net_impact,
                    "last_interaction_days": (time.time() - metrics.last_interaction)
                    / 86400,
                }
            )

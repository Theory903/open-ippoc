...
class BodyAdapter(IPPOC_Tool):
    ...
    """
    def __init__(self):
        super().__init__(name="body", domain="body", role=CognitiveRole.ACTOR)
        self._populate_openclaw_tools()

    def _populate_openclaw_tools(self):
        """Populate tool registry with discovered OpenClaw skills"""
        try:
            scanner_mod = importlib.import_module("cortex.plugins.openclaw.proprioception_scanner")
            scanner = scanner_mod.get_scanner()
            ...
            # Normal if OpenClaw plugin is missing
            pass
    ...
    def _violates_canon(self, envelope: ToolInvocationEnvelope) -> bool:
        ...
        from ippoc.cortex.core.intents import Intent
    ...
    async def _execute_openclaw_skill(self, envelope: ToolInvocationEnvelope) -> ToolResult:
        ...
        try:
            adapter_mod = importlib.import_module("cortex.plugins.openclaw.openclaw_adapter")
        except ImportError:
            return ToolResult(
                success=False,
                output="OpenClaw plugin missing",
        ...
        kernel_status = adapter_mod.get_kernel_status()
        ...
        try:
            ...
            result = await adapter_mod.send_directive_to_kernel(directive)

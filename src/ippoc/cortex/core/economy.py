...
class EconomyManager:
    ...
    def _state_to_dict(self) -> Dict[str, Any]:
        # Optimization: Avoid dataclasses.asdict for performance, use manual dict creation with shallow copies
        s = self.state
        return {
            "budget": s.budget,
            "reserve": s.reserve,
            ...
            "tool_stats": {
                ...
                for k, v in s.tool_stats.items()
            },
            "events": [e.copy() if isinstance(e, dict) else e for e in s.events],
            "last_tick": s.last_tick,
            "last_earning_timestamp": s.last_earning_timestamp,
        }

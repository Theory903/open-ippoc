...
class AdaptiveRAGRouter:
    ...
    # Static string tuples for optimized matching
    COMPLEX_INDICATORS = (
    ...
    SELF_REF_KEYWORDS = ('you', 'your', 'identity', 'memory')
...
Also categorize it into: research, technical, memory, factual, conversational, system, general"""),
    ...
    def _assess_safety_risk(self, query: str) -> float:
        ...
        if any(word in query_lower for word in self.SELF_REF_KEYWORDS):
            risk_score += 0.3
    ...
    def _assess_speed_need(self, query: str, context: Dict) -> float:
        ...
        interactive_score = 0.8 if any(word in str(context.get('mode', '')).lower() for word in self.INTERACTIVE_INDICATORS) else 0.3

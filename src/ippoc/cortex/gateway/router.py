...
Also categorize it into: research, technical, memory, factual, conversational, system, general"""),
    ...
    _COMPLEX_DENOMINATOR = float(len(_COMPLEX_INDICATORS))
    ...
    _VIOLATION_DENOMINATOR = float(len(_VIOLATION_KEYWORDS))
    _SELF_REFERENCE_WORDS = ('you', 'your', 'identity', 'memory')
    ...
    def _assess_complexity(self, query: str) -> float:
        ...
        for word in self._COMPLEX_INDICATORS:
            if word in query_lower:
                matches += 1
        keyword_score = matches / self._COMPLEX_DENOMINATOR
    ...
    def _assess_safety_risk(self, query: str) -> float:
        ...
        for word in self._VIOLATION_KEYWORDS:
            if word in query_lower:
                matches += 1
        risk_score = matches / self._VIOLATION_DENOMINATOR
        
        for word in self._SELF_REFERENCE_WORDS:
            if word in query_lower:
                risk_score += 0.3
                break

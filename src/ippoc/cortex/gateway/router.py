...
Also categorize it into: research, technical, memory, factual, conversational, system, general""",
    ...
    def _assess_complexity(self, query: str) -> float:
        ...
        for word in complex_indicators:
            if word in query_lower:
                matches += 1
        keyword_score = matches / len(complex_indicators)
    ...
    def _assess_safety_risk(self, query: str) -> float:
        ...
        for word in violation_keywords:
            if word in query_lower:
                matches += 1
        risk_score = matches / len(violation_keywords)

        # Self-reference queries (potentially sensitive)
        sensitive_words = ("you", "your", "identity", "memory")
    ...
    def _assess_speed_need(self, query: str, context: Dict) -> float:
        ...
        context_mode_lower = str(context.get("mode", "")).lower()

        interactive_score = 0.3
        for word in interactive_indicators:
            if word in context_mode_lower:
        ...
        realtime_score = 0.4
        for word in realtime_indicators:
            if word in query_lower:
                realtime_score = 0.9
                break
    ...
    def _classify_intent(self, query: str) -> str:
        ...
        for word in ("code", "implement", "program", "script", "function"):
            if word in query_lower:
                return "technical"

        # Memory/Context intent
        for word in ("remember", "recall", "who", "what", "when", "relationship"):
            if word in query_lower:
                return "memory"

        # Simple/Factual intent
        for word in ("what is", "how to", "define", "explain"):
            if word in query_lower:
                return "factual"

        # Conversational intent
        for word in ("hello", "hi", "hey", "goodbye", "thanks"):
            if word in query_lower:
                return "conversational"

        # System/Status intent
        for word in ("status", "version", "ping", "health", "running"):
            if word in query_lower:
                return "system"

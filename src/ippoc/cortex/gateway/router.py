...
class AdaptiveRAGRouter:
    ...
    # Static sets for fast O(1) keyword lookups/checks in string
    COMPLEX_INDICATORS = ('research', 'analyze', 'implement', 'compare', 'evaluate',
                          'strategy', 'architecture', 'design', 'optimize', 'debug')
    QUESTION_INDICATORS = ('how', 'why', 'explain', 'compare')

    VIOLATION_KEYWORDS = ('delete', 'destroy', 'harm', 'illegal', 'bypass', 'hack',
...
Also categorize it into: research, technical, memory, factual, conversational, system, general"""),
    ...
    def _assess_complexity(self, query: str) -> float:
        ...
        match_count = 0
        for w in self.COMPLEX_INDICATORS:
            if w in query_lower:
                match_count += 1
        keyword_score = match_count / len(self.COMPLEX_INDICATORS)
    ...
    def _assess_safety_risk(self, query: str) -> float:
        ...
        match_count = 0
        for w in self.VIOLATION_KEYWORDS:
            if w in query_lower:
                match_count += 1
        risk_score = match_count / len(self.VIOLATION_KEYWORDS)
        
        # Self-reference queries (potentially sensitive)
        for w in self.SELF_REF_KEYWORDS:
            if w in query_lower:
                risk_score += 0.3
                break
    ...
    def _assess_speed_need(self, query: str, context: Dict) -> float:
        ...
        for w in self.INTERACTIVE_INDICATORS:
            if w in context_mode_lower:
        ...
        # Real-time context
        realtime_score = 0.4
        for w in self.REALTIME_INDICATORS:
            if w in query_lower:
                realtime_score = 0.9
                break
    ...
    def _classify_intent(self, query: str) -> str:
        ...
        for w in self.INTENT_RESEARCH:
            if w in query_lower: return 'research'
            
        # Technical/Code intent
        for w in self.INTENT_TECHNICAL:
            if w in query_lower: return 'technical'
            
        # Memory/Context intent
        for w in self.INTENT_MEMORY:
            if w in query_lower: return 'memory'
            
        # Simple/Factual intent
        for w in self.INTENT_FACTUAL:
            if w in query_lower: return 'factual'
            
        # Conversational intent
        for w in self.INTENT_CONVERSATIONAL:
            if w in query_lower: return 'conversational'
            
        # System/Status intent
        for w in self.INTENT_SYSTEM:
            if w in query_lower: return 'system'

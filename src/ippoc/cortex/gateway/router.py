...
Also categorize it into: research, technical, memory, factual, conversational, system, general"""),
    ...
    def _classify_intent(self, query: str) -> str:
        ...
        for word in self.RESEARCH_INTENTS:
            if word in query_lower: return 'research'
        for word in self.TECHNICAL_INTENTS:
            if word in query_lower: return 'technical'
        for word in self.MEMORY_INTENTS:
            if word in query_lower: return 'memory'
        for word in self.FACTUAL_INTENTS:
            if word in query_lower: return 'factual'
        for word in self.CONVERSATIONAL_INTENTS:
            if word in query_lower: return 'conversational'
        for word in self.SYSTEM_INTENTS:

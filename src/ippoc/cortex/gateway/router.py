...
class AdaptiveRAGRouter:
    ...
    def __init__(self, llm: Runnable = None, embeddings: Embeddings = None, vectorstore: VectorStore = None):
        ...
        # Performance optimization: Tuple constants for string matching overhead reduction
        self.COMPLEX_INDICATORS = (
            'research', 'analyze', 'implement', 'compare', 'evaluate',
            'strategy', 'architecture', 'design', 'optimize', 'debug'
        )
        self.QUESTION_INDICATORS = ('how', 'why', 'explain', 'compare')
        self.VIOLATION_KEYWORDS = (
            'delete', 'destroy', 'harm', 'illegal', 'bypass', 'hack',
            'override', 'ignore', 'disable', 'remove'
        )
        self.SELF_REF_KEYWORDS = ('you', 'your', 'identity', 'memory')
        self.INTERACTIVE_INDICATORS = ('chat', 'conversation', 'discuss', 'talk')
        self.REALTIME_INDICATORS = ('now', 'immediately', 'quick', 'fast')

        self.RESEARCH_WORDS = ('research', 'study', 'analyze', 'investigate')
        self.TECHNICAL_WORDS = ('code', 'implement', 'program', 'script', 'function')
        self.MEMORY_WORDS = ('remember', 'recall', 'who', 'what', 'when', 'relationship')
        self.FACTUAL_WORDS = ('what is', 'how to', 'define', 'explain')
        self.CONVERSATIONAL_WORDS = ('hello', 'hi', 'hey', 'goodbye', 'thanks')
        self.SYSTEM_WORDS = ('status', 'version', 'ping', 'health', 'running')
...
Also categorize it into: research, technical, memory, factual, conversational, system, general"""),
    ...
    def _assess_complexity(self, query: str) -> float:
        ...
        for word in self.COMPLEX_INDICATORS:
            if word in query_lower:
                complex_count += 1
        keyword_score = complex_count / len(self.COMPLEX_INDICATORS)
    ...
    def _assess_safety_risk(self, query: str) -> float:
        ...
        for word in self.VIOLATION_KEYWORDS:
            if word in query_lower:
                violation_count += 1
        risk_score = violation_count / len(self.VIOLATION_KEYWORDS)
        
        # Self-reference queries (potentially sensitive)
        for word in self.SELF_REF_KEYWORDS:
            if word in query_lower:
                risk_score += 0.3
                break
    ...
    def _classify_intent(self, query: str) -> str:
        ...
        for word in self.TECHNICAL_WORDS:
            if word in query_lower:
                return 'technical'
        ...
        for word in self.CONVERSATIONAL_WORDS:
            if word in query_lower:

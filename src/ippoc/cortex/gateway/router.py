...
Also categorize it into: research, technical, memory, factual, conversational, system, general"""),
    ...
    def _assess_complexity(self, query: str) -> float:
        ...
        for word in self.COMPLEX_INDICATORS:
            if word in query_lower:
                keyword_hits += 1
        keyword_score = keyword_hits / len(self.COMPLEX_INDICATORS)
    ...
    def _assess_safety_risk(self, query: str) -> float:
        ...
        for word in self.VIOLATION_KEYWORDS:
            if word in query_lower:
                violation_hits += 1
        risk_score = violation_hits / len(self.VIOLATION_KEYWORDS)
    ...
    def _classify_intent(self, query: str) -> str:
        ...
        for word in self.INTENT_RESEARCH:
            if word in query_lower:
                return 'research'

        for word in self.INTENT_TECHNICAL:
            if word in query_lower:
                return 'technical'

        for word in self.INTENT_MEMORY:
            if word in query_lower:
                return 'memory'

        for word in self.INTENT_FACTUAL:
            if word in query_lower:
                return 'factual'

        for word in self.INTENT_CONVERSATIONAL:
            if word in query_lower:
                return 'conversational'

        for word in self.INTENT_SYSTEM:
            if word in query_lower:
                return 'system'
